...
InsSQL = 'INSERT INTO podstprognoz (id, podst_id, StartDt, PikDt, EndDt, pik, Smax) '+
         'VALUES(NULL, :pid, :sd, :pd, :ed, :p, :sm)';
SelLastId = 'SELECT  LAST_INSERT_ID() as lid';
...
//  ZTService.ADOConnection1.BeginTrans;
  try
    //чистим если уже были данные за этот период
    Cmnd.Connection := ZTService.ADOConnection1;
    CmndTI.Connection := ZTService.ADOConnection1;
    CmndTI.CommandText:=InsTI;
    DS.Connection := ZTService.ADOConnection1;
    Cmnd.CommandText:=DelSQL;
...
//        CmndTI.CommandText:=InsTI;
        ...
        CmndTI.Parameters.ParamByName('pp_id').Value := lid;
...
begin
  for i := 0 to TransInfoList.Count-1 do
    TTransInfoRorPrognozItem(TransInfoList.Items[i]).Free;
  TransInfoList.Clear;
  TransInfoList.Free;
  inherited;
end;
