  ...
  try
    ...
    begin
        ...
        try
          ...
          begin
            ...
            try
                  ...
                  begin
                   ...
                   begin
                     ...
                     begin
                       ...
                       WriteLog('Ошибка данных ОИК', oik);
                   end else
                     WriteLog('Ошибка данных ОИК', oik);
                  end;
                end;
              ...
              WriteLog(' '+IntToStr(nl.Count)+' перегрузок, нагрузка '+IntToStr(round(nl.zagr))+'%', oik);
              try
              Lock.Enter;
          ...
          //Тут сохранять в базу
          WriteLog('Прогноз перегруза: '+IntToStr(npl.Count),oik);
      ...
      begin
       ...
       on e: Exception do
          WriteLog('Ошибка при обработке '+pName+': '+e.message, oik);
      end;
      finally
       WriteLog(pName+' }', oik);
       PodstDS.Next;
...
//            Threads[I].Start;
        ...
        begin
          Threads[I].Start;
          WaitForSingleObject(Handles[I], INFINITE); // по очереди
        end;

//         Wait until threads terminate - паралельно
