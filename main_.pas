...
begin
  ...
  begin
      ...
      begin
        WriteLog('ТМ-сервер '+ oikName + ' подключен');

        hr := CoInitialize(nil);
        try
        WriteLog('Загрузка '+poNm+' стартует');
        DataProc(poid, oikName);
          ...
          WriteLog('Загрузка '+poNm+' завершена');
        end;

        WriteLog('ТМ-сервер ' + oikName + ' отсоединен');
      end
      else
      begin
        WriteLog('ОШИБКА! ТМ-сервер ' + oikName + ' не подключен');
      end;
  end
...
//        hr := CoInitialize(nil);
//        try
//        WriteLog('Загрузка '+poName+' стартует');
//        DataProc(po_id, oik);
//        finally
//          tmClose;
//          case hr of
...
//              CoUninitialize;
//          end;
//          WriteLog('Загрузка '+poName+' завершена');
//        end;
//
